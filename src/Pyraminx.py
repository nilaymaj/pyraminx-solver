...
class Pyraminx:
  ...
  # The below method does NOT compare the corresponding tips
  def eq(self, other):
    for i in range(1, 5):
      face1 = self.face(i)
      face2 = other.face(i)
  ...
  # Returns a copy of this pyraminx
  def copy(self):
    newP = pUtils.solvedPyraminx()
    newP.red_face = copy.deepcopy(self.red_face)
    newP.yellow_face = copy.deepcopy(self.yellow_face)
    newP.green_face = copy.deepcopy(self.green_face)
    newP.blue_face = copy.deepcopy(self.blue_face)
    return newP

  # Returns a copy of the modified pyraminx without affecting the original
  def makeMove(self, moveId):
    newP = self.copy()
    newP.move(moveId)
    return newP
